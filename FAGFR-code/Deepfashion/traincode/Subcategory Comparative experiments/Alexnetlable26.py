...
from model_group_api_lsk_encoder import Net
from models import Vgg, Alex_Net
...
class Config:
    ...
    # 默认预训练权重文件路径
    pretrained_model_path = os.path.join(root_dir, "pretrained", "Alexnet.pth")
...
def build_net():
    ...
    return Alex_Net(num_classes=Config.num_classes)
...
#############################
# 4. 新的评估函数：计算每个属性在 top-3 和 top-5 下的正确率
#############################
...
def main():
    ...
    # 训练过程，每个 epoch 结束后输出26个标签的 top-3 和 top-5 正确率
